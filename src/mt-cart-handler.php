...
function mt_handle_cart() {
	...
	if ( ! isset( $_POST['mt_submit'] ) ) {
		...
		if ( ! $email_valid || ! isset( $_POST['mt_fname'] ) || '' === $_POST['mt_fname'] || ! isset( $_POST['mt_lname'] ) || '' === $_POST['mt_lname'] || ! isset( $_POST['mt_email'] ) || '' === $_POST['mt_email'] ) {
...
function mt_create_payment( $post ) {
	...
	update_post_meta( $payment_id, '_last_name', sanitize_text_field( $post['mt_lname'] ) );
	if ( isset( $options['mt_ticket_handling'] ) && is_numeric( $options['mt_ticket_handling'] ) ) {
		update_post_meta( $payment_id, '_ticket_handling', $options['mt_ticket_handling'] );
	}
	$email = $post['mt_email'];
	update_post_meta( $payment_id, '_email', sanitize_email( $email ) );
	$phone = ( isset( $post['mt_phone'] ) ) ? $post['mt_phone'] : '';
	update_post_meta( $payment_id, '_phone', sanitize_text_field( $phone ) );
	if ( $purchaser ) {
		update_user_meta( $purchaser, 'mt_phone', sanitize_text_field( $phone ) );
	}
	$vat = ( isset( $post['mt_vat'] ) ) ? $post['mt_vat'] : '';
	update_post_meta( $payment_id, '_vat', sanitize_text_field( $vat ) );
	if ( is_user_logged_in() ) {
		update_user_meta( $purchaser, 'mt_vat', sanitize_text_field( $vat ) );
	}
	$purchased = ( isset( $post['mt_cart_order'] ) ) ? $post['mt_cart_order'] : false;
	$paid      = mt_calculate_cart_cost( $purchased, $payment_id );
	...
	if ( isset( $options['mt_handling'] ) && is_numeric( $options['mt_handling'] ) ) {
		...
		update_post_meta( $payment_id, '_mt_handling', $handling_total );
	}
	if ( isset( $options['mt_shipping'] ) && 'postal' === $post['ticketing_method'] ) {
		$paid = $paid + $options['mt_shipping'];
		update_post_meta( $payment_id, '_mt_shipping', $options['mt_shipping'] );
	}
	update_post_meta( $payment_id, '_total_paid', $paid );
	$payment_status = ( 0 === (float) $paid ) ? 'Completed' : 'Pending';
	update_post_meta( $payment_id, '_is_paid', $payment_status );
	if ( is_user_logged_in() && ! is_admin() && '' !== trim( $options['mt_members_discount'] ) ) {
		update_post_meta( $payment_id, '_discount', $options['mt_members_discount'] );
	}
	update_post_meta( $payment_id, '_gateway', $gateway );
	update_post_meta( $payment_id, '_purchase_data', $purchased );
	// Debugging.
	mt_debug( print_r( $purchased, 1 ), 'Purchase Data saved at nav to payment screen', $payment_id );
	mt_debug( print_r( $post, 1 ), 'Data passed from client at nav to payment screen', $payment_id );
	update_post_meta( $payment_id, '_ticketing_method', $post['ticketing_method'] );
	/**
	...
	if ( $payment_id ) {
		$wp_post    = get_post( $payment_id );
		$post_title = $wp_post->post_title;
		$meta_name  = get_post_meta( $payment_id, '_first_name', true ) . ' ' . get_post_meta( $payment_id, '_last_name', true );
		$post_data  = array(
			'ID'        => $payment_id,
			'post_name' => 'mt_payment_' . $payment_id,
		);
		// If the purchaser changed their name after purchase creation, update to the later values.
		if ( $meta_name !== $post_title ) {
			$post_data['post_title'] = $meta_name;
		}
	...
	update_post_meta( $payment_id, '_receipt', $receipt_id );
	if ( 'publish' === $status ) {
		wp_update_post(
			array(
				'ID'          => $payment_id,
...
function mt_check_inventory( $event_id, $type = '', $virtual = 'auto' ) {
	...
	if ( 'virtual' === $is_virtual && ( 'auto' === $virtual || true === $virtual ) ) {
		...
		if ( '' !== $type ) {
			...
			$current_virtual = 0;
			if ( is_array( $virtual_inventory ) ) {
				foreach ( $virtual_inventory as $t => $quantity ) {
 ...
 * @return boolean
 */
function mt_ticket_exists( $payment_id, $ticket_id ) {
	global $wpdb;
	$value = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->postmeta WHERE post_id = %d AND meta_value = %s", $payment_id, $ticket_id ) );
 ...
 * @return string
 */
function mt_generate_ticket_id( $payment_id, $event_id, $type, $i, $price ) {
	// hash data.
	$hash = md5( $payment_id . $type . $i . $price . $event_id );
	// reduce to 13 chars.
 ...
 * @param int   $payment_id Payment ID to compare against.
 *
 * @return float Checked value
 */
function mt_check_payment_amount( $price, $payment_id ) {
	$total_paid = get_post_meta( $payment_id, '_total_paid', true );
	$donation   = get_post_meta( $payment_id, '_donation', true );
