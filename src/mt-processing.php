...
function mt_registration_fields( $form, $has_data, $data, $public = 'admin' ) {
	...
	$checked       = '';
	$notes         = '';
	if ( $has_data === true && property_exists( $data, 'event_post' ) ) {
...
function mt_prices_table( $registration = array() ) {
	...
	if ( $counting == 'discrete' ) {
		...
		$value           = ( isset( $registration['total'] ) && $registration['total'] != 'inherit' ) ? $registration['total'] : $tickets;
 ...
 *
 * @param $name Key for stored label
 *
...
*/
/**
 * Problem: need to make sure that the total number of items remains constant and items remain grouped correctly.
 */
