...
function mt_select_events() {
	...
	foreach ( $posts as $post ) {
		...
		if ( is_array( $event_data ) ) {
			$event_date   = strtotime( $event_data['event_begin'] . ' ' . $event_data['event_time'] );
...
function mt_format_if_datetime( $label ) {
	...
	if ( 4 === count( $test ) ) {
		...
		if ( $label === $formatted ) {
			...
			$date        = mt_date( 'Y-m-d', $ts );
			$time        = mt_date( 'H:i:s', $ts );
			return mt_date( $date_format . ' ' . $time_format, strtotime( $date . ' ' . $time ), false );
		}
	}
