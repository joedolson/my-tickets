...
function mt_purchases( $event_id, $options = array( 'include_failed' => false ) ) {
	...
	$report       = array(
		'html' => array( 'Completed' => array(), 'Pending' => array(), 'Refunded' => array(), 'Failed' => array(), 'Reserved' => array(), 'Turned Back' => array(), 'Waiting List' => array() ),
		'csv'  => array( 'Completed' => array(), 'Pending' => array(), 'Refunded' => array(), 'Failed' => array(), 'Reserved' => array(), 'Turned Back' => array(), 'Waiting List' => array() )
	);
	$total_tickets = 0;
	$total_income  = 0;
	$alternate    = 'even';
	foreach ( $query as $payment ) {
		foreach ( $payment as $purchase_id => $details ) {
			...
			if ( $options['include_failed'] == false && ( $status == 'Failed' || $status == 'Refunded' || $status == 'Turned Back' ) ) {
				continue;
			}
			$types        = '';
			$ticket_count = 0;
...
function mt_download_csv_event() {
	if (
		...
		$csv =   __( 'Last Name', 'my-tickets' ) . ","
		       . __( 'First Name', 'my-tickets' ) . ","
		       . __( 'Email', 'my-tickets' ) . ","
...
function mt_mass_email( $event_id = false ) {
	...
	if ( $event_id ) {
		...
		add_post_meta( $event_id, '_mass_email', array( 'subject' => $subject, 'body' => $body ) );
		if ( !$body || ! $subject ) {
