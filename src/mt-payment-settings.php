...
function mt_payment_settings() {
	...
	$alert = '';
	$testing_mode = ( $options['mt_use_sandbox'] == 'true' ) ? true : false;
	if ( $testing_mode ) {
							...
							<div class="inside">
								<ul>
									<li><label for="mt_currency"><?php _e( 'Currency:', 'my-tickets' ); ?></label>
										<?php $mt_currency_codes = mt_currency();
					...
					<input type='checkbox' id='mt_gateway_$gateway' name='mt_gateway[]' value='$gateway'" . $gateway_enabled . " /> <label for='mt_gateway_$gateway'>$fields[label]</label>
										...
										foreach ( $settings as $key => $label ) {
											if ( is_array( $label ) ) {
												...
												$checked      = ( $input_type == 'checkbox' && ( isset( $options['mt_gateways'][ $gateway ][ $key ] ) && $options['mt_gateways'][ $gateway ][ $key ] == $label['value'] ) ) ? 'checked="checked"' : '';
										...
										$notes    = ( isset( $fields['note'] ) ) ? "<p>" . wp_kses( $fields['note'], array( 'strong'=>array(), 'code'=>array(), 'em'=>array(), 'a'=>array( 'href' ) ) ) . "</p>" : '';
...
function mt_zerodecimal_currency() {
	...
	if ( isset( $data['zerodecimal'] ) && $data['zerodecimal'] == true ) {
		return true;
	}
