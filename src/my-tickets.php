...
function mt_activation() {
	...
	if ( ! wp_next_scheduled( 'my_tickets_daily_cron' ) ) {
		wp_schedule_event( current_time( 'timestamp' ), 'daily', 'my_tickets_daily_cron' ); // phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp.Requested
	}
}

add_action( 'my_tickets_daily_cron', 'my_tickets_daily_cron_actions' );
/**
 * Daily cron job to remove expired anonymous shopping cart data.
 */
function my_tickets_daily_cron_actions() {
	$carts = mt_find_carts();
...
function mt_public_enqueue_scripts() {
	...
	if ( SCRIPT_DEBUG ) {
		$payments_url = plugins_url( 'js/jquery.payments.js', __FILE__ );
		$public_url   = plugins_url( 'js/jquery.public.js', __FILE__ );
	} else {
		$payments_url = plugins_url( 'js/jquery.payments.min.js', __FILE__ );
		$public_url   = plugins_url( 'js/jquery.public.min.js', __FILE__ );
	}
	wp_enqueue_script( 'mt.payments', $payments_url, array( 'jquery', 'wp-a11y' ), $version );
