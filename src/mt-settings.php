...
/**
 * Custom sanitization for My Tickets templates. Sanitizes with wp_kses_post,
 * then restores single quote attribute wrappers.
 *
 * @param string $message Original value passed from $_POST.
 *
 * @return string
 */
function mt_custom_sanitize_messages( $message ) {
	$modified = wp_kses_post( wp_unslash( $message ) );
	$original = $message;

	return mt_kses_replacement( $modified, $original );
}
 ...
 */
function mt_kses_replacement( $modified, $original ) {
...
function mt_wp_enqueue_scripts() {
	...
	if ( 'mt-payments' === $current_screen->post_type ) {
		wp_enqueue_script( 'mt.payments', plugins_url( 'js/jquery.payments.js', __FILE__ ), array( 'jquery', 'wp-a11y' ), $version );
		wp_localize_script(
			'mt.payments',
			'mt_data',
			array(
...
function mt_ajax_move_ticket() {
	...
	if ( ! $event_id || ! ( $target || $type ) || ! $ticket ) {
		wp_send_json(
			array(
				'success'  => 0,
...
function mt_report_scripts() {
	...
	if ( isset( $_GET['mt-event-report'] ) && isset( $_GET['mt_print'] ) ) {
		wp_enqueue_script( 'mt.sortable', plugins_url( 'js/sortable.js', __FILE__ ), array(), $version );
